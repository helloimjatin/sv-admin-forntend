    ...
    <div className="space-y-6 w-full">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          ...
          <PageHeader title={resolvedEditId ? "Edit Notification" : "Create Notification"} />
          {(draftSavedAt || autoSaving) && (
            <p className="text-xs text-primary font-medium flex items-center gap-1 mt-1" aria-live="polite">
        ...
        <div className="flex flex-wrap gap-2">
          <button
            ...
            disabled={submitting}
            className="rounded-lg border border-outline-variant px-5 py-2.5 text-xs font-semibold uppercase tracking-wide hover:bg-surface-elevated disabled:opacity-50"
          >
            Cancel
          </button>
          <button
          ...
          <button
            type="button"
            onClick={() => { runValidation(false); setPreviewOpen(true); }}
            disabled={submitting}
            className="rounded-lg border border-outline-variant px-5 py-2.5 text-xs font-semibold uppercase tracking-wide hover:bg-surface-elevated disabled:opacity-50"
          >
            Preview
          ...
          <button
            ...
            onClick={handleTest}
            disabled={submitting}
            className="rounded-lg border border-primary text-primary px-5 py-2.5 text-xs font-semibold uppercase tracking-wide hover:bg-primary-fixed/30 disabled:opacity-50"
          >
            Test
          </button>
          {!form.send_now ? (
            ...
            <div className="flex flex-wrap gap-2">
              {(
                [
                  ...
                  { label: "Expired Subscribers", value: "expired" as const },
                  { label: "Selected Users", value: "custom_segment" as const },
                ] satisfies { label: string; value: AudienceType }[]
              ).map((seg) => {
